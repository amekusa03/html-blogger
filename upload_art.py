...
def move_history_file(src_path):
    ...
    try:
        ...
        logger.info(f"履歴保存: {src_path.name}")
        return True
    except Exception as e:
        logger.error(f"履歴保存失敗: {src_path.name} - {e}")
        return False
...
def ready_upload():
    ...
    if blog_id == 'あなたのブログID' or not blog_id:
        logger.error("BLOG_ID が設定されていません。config.json5 の [UPLOAD_ART] セクションを確認してください。")
        return False
...
def run(result_queue):
    ...
    # 処理対象のファイルを input_dir から取得し、名前順でソート
    files_to_process = sorted([p for p in Path(input_dir).rglob('*') if p.is_file() and p.suffix.lower() in html_extensions])
    ...
    for src_path in files_to_process:
        if processed_count >= max_posts_per_run:
            logger.info(f"1回の実行上限 ({max_posts_per_run} 件) に達しました。")
            break
