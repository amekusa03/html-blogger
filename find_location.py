...
geocode_debug = config['find_location']['geocode_debug']
...
def find_location_in_html(files):
    ...
    html_text_for_tokenize = re.sub(r'[^\u3040-\u309F\u30A0-\u30FF\u4E00-\u9FFF\s]', '', html_text)
    ...
    for spot in spot_candidates:
        ...
        if spot_lower in location_cache:
            ...
            if cached_lat and cached_lon:
                # キャッシュから取得成功
                logger.info(f"キャッシュから取得: {spot} -> ({cached_lat}, {cached_lon})")
        ...
        try:
            for attempt in range(geocode_retries):
                try:
                    ...
                    if geocode_debug:
                        logger.debug(f"(デバッグモード) ジオコーディング検索スキップ: {spot}")
                    else:  
                        location = geolocator.geocode(spot, language='ja', timeout=geocode_timeout)
                    time.sleep(geocode_wait) # Nominatimの利用規約
                    
                    if location:
                        # ジオコーディング検索成功
                        logger.info(f"ジオコーディング成功: {spot} -> ({location.latitude}, {location.longitude})")
                        # キャッシュ保存
                        save_location_cache( spot, location.latitude, location.longitude)
                        location_cache[spot_lower] = (spot, location.latitude, location.longitude)
                        # 結果設定
...
def run(result_queue):
    ...
    files_to_process = [p for p in Path(input_dir).rglob('*') if p.is_file() and p.suffix.lower() in (html_extensions)]
